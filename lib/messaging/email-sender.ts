...
async function updateLog(
  ...
  await supabase.from('message_logs').update(payload).eq('id', messageLogId)
}

// Lightweight transactional email (no message_log — for operational emails like form links)
export async function sendTransactionalEmail(input: {
  ...
  const fromName = settings?.resend_from_name || process.env.RESEND_FROM_NAME || ''
  ...
  const result: any = await resend.emails.send({
    ...
    html: input.html,
    text: input.html.replace(/<[^>]+>/g, ' ').trim(),
  })
