...
UPDATE TO authenticated USING (user_id = auth.uid()) WITH CHECK (
    user_id = auth.uid()
    AND salon_id = public.get_user_salon_id() -- Nie można zmienić salonu
  );
...
-- COMMENTS
-- ========================================
COMMENT ON POLICY "Users can view salon profiles" ON public.profiles IS 'Użytkownicy mogą widzieć tylko profile z tego samego salonu.';
COMMENT ON POLICY "Users can update their own profile" ON public.profiles IS 'Użytkownicy mogą edytować tylko swój własny profil (nie mogą zmienić salon_id).';
