...
describe('BooksyProcessor', () => {
  ...
  it('does not consume the preceding phone number as the day when date line starts with weekday', () => {
    const supabaseMock = {
      from: () => new BookingQueryMock({ data: null, error: null }),
    }
    ...
    const parsed = processor.parseEmail(
      'Julia Kowalczyk: nowa rezerwacja',
      [
        'Julia Kowalczyk',
        '',
        '695 097 077',
        '',
        'piatek, 8 maja 2026, 15:30 - 17:35',
        '',
        'Manicure ANIA: Przedluzanie paznokci S/M',
        '',
        '170,00 zl+,',
        '',
        'pracownik:',
    ...
    expect(parsed).toMatchObject({
      type: 'new',
      clientName: 'Julia Kowalczyk',
      clientPhone: '695097077',
