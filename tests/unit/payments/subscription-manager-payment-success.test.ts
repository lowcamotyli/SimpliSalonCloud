...
describe('SubscriptionManager.handlePaymentSuccess', () => {
  it('retry with existing invoice is idempotent and does not insert duplicate invoice', async () => {
    ...
    expect(counters.invoiceInserts).toBe(0)
    expect(counters.subscriptionUpdates).toBe(2)
    expect(counters.enablePlanFeaturesCalls).toBe(2)
  })

  it('invoice failure blocks activation and feature enablement (failure-order safety)', async () => {
    ...
    expect(counters.invoiceInserts).toBe(1)
    expect(counters.subscriptionUpdates).toBe(0)
    expect(counters.enablePlanFeaturesCalls).toBe(0)
  })
})

describe('SubscriptionManager.createInvoice', () => {
  it('concurrent retries are race-safe when second insert returns 23505', async () => {
    ...
    expect(invoiceInsertCalls).toBe(2)
  })
})
