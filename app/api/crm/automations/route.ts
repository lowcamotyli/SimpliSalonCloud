...
async function getPlanAndLimit(supabase: any, salonId: string) {
  ...
  return { plan, activeLimit: getAutomationLimitForPlan(plan) }
}
...
async function enforceActiveLimit(supabase: any, salonId: string, activeLimit: number | null) {
...
export async function GET(request: NextRequest) {
  try {
    ...
    const [listResult, meta] = await Promise.all([
      ...
      getPlanAndLimit(supabase as any, parsedQuery.data.salonId),
    ])
    ...
    const activeCount = (listResult.data || []).filter((a: any) => a.is_active).length
...
export async function POST(request: NextRequest) {
  try {
    ...
    validateTriggerConfig(payload.triggerType, payload.triggerConfig)
    await ensureTemplateLinkage(supabase as any, payload.salonId, payload.templateId, payload.channel)

    const { activeLimit, plan } = await getPlanAndLimit(supabase as any, payload.salonId)
    if (payload.isActive) {
      await enforceActiveLimit(supabase as any, payload.salonId, activeLimit)
    }
    ...
    return NextResponse.json({ automation: data, meta: { plan, activeLimit } }, { status: 201 })
  } catch (error) {
